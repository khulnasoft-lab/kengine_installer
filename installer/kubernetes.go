...
import (
	...
	"github.com/khulnasoft-lab/kengine_installer/agent"
)
...
func (ki *KubernetesInstaller) Delete() error {
	cmd := exec.Command("helm", "delete kengine-agent")
	return cmd.Run()
}

func (ki *KubernetesInstaller) Install() error {
	cmd := exec.Command("helm", fmt.Sprintf(`install kengine-agent
		--set image.tag=%s
		--set managementConsoleUrl=%s
		--set kengineKey=%s`, ki.previous.ImageTag, ki.consoleip, ki.apikey))
	return cmd.Run()
}
